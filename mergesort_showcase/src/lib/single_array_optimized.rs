...
fn merge(
    ...
    while left_lo < left_hi && right_lo < right_hi {
        ...
        helper_buf.set(buf_lo, element);
        buf_lo += 1;
    }
    while left_lo < left_hi {
        helper_buf.set(buf_lo, array.read(left_lo));
        left_lo += 1;
        buf_lo += 1;
    }
    while right_lo < right_hi {
        helper_buf.set(buf_lo, array.read(right_lo));
        right_lo += 1;
        buf_lo += 1;
    }
}

fn merge_sort(arr: Array, mut lo: usize, hi: usize, helper_buf: Array) {
    let mid = lo + (hi - lo) / 2;
    if mid == lo {
        return;
    }
    ...
    merge(arr, lo, mid, hi, helper_buf);
    while lo < hi {
        arr.set(lo, helper_buf.read(lo));
        lo += 1;
    }
}

fn _merge_sort_parallel(arr: Array, mut lo: usize, hi: usize, helper_buf: Array, threshold: usize) {
    let mid = lo + (hi - lo) / 2;
    if mid == lo {
        return;
    }
...
pub struct SingleArrayOptimized;
impl Sort for SingleArrayOptimized {
...
fn test_seq() {
    ...
    SingleArrayOptimized::sort(&mut a, 5);
