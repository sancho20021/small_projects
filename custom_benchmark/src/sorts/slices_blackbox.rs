...
fn copy(from: &[i32], mut from_lo: usize, from_hi: usize, to: &mut [i32], mut to_lo: usize) {
    while from_lo < from_hi {
        to[std::hint::black_box(to_lo)] = from[std::hint::black_box(from_lo)];
        from_lo += 1;
        to_lo += 1;
    }
}
...
fn merge(left: &[i32], right: &[i32], helper_buf: &mut [i32]) {
    ...
    while left_i < left.len() && right_i < right.len() {
        left_i = std::hint::black_box(left_i);
        right_i = std::hint::black_box(right_i);
        helper_buf_i = std::hint::black_box(helper_buf_i);
        let element: i32;
