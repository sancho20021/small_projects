...
use crate::sorts::verus_no_ghost_muninit::permissions_array::Array;

pub struct ArrayForSorting<T> {
    pub array: Arc<Array<T>>,
}

struct PCell<T>(UnsafeCell<MaybeUninit<T>>);
...
impl<T> PCell<T> {
    pub fn new(x: T) -> Self {
        PCell(UnsafeCell::new(MaybeUninit::new(x)))
    }

    pub fn replace(&self, x: T) -> T {
        unsafe {
            let mut m = MaybeUninit::new(x);
            std::mem::swap(&mut m, &mut *self.0.get());
            m.assume_init()
        }
    }
...
fn _merge_sort_parallel(
    ...
    let left_perms = std::thread::spawn(move || -> Result<(), ()> {
        let t = _merge_sort_parallel(arr_r1, lo, mid, out_arr_r1, out_lo, threshold);
        if t.is_err() { Err(()) } else { Ok(()) }
    });

    match _merge_sort_parallel(arr_r2, mid, hi, out_arr_r2, out_mid, threshold) {
        Ok(()) => {}
        Err(_) => {
            return Err(());
        }
...
fn test_par_array() {
    ...
    _merge_sort_parallel(Arc::clone(&arr), 0, len, Arc::clone(&out_arr), 0, 2).unwrap();
...
mod permissions_array {
    ...
    use crate::sorts::verus_no_ghost_muninit::PCell;
