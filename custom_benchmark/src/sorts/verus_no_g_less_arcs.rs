...
use crate::sorts::verus_no_g_less_arcs::permissions_array::Array;
...
fn _merge_sort_parallel(
    ...
    merge(&arr, lo, mid, mid, hi, &out_arr, out_lo);
    let arr_d = &*arr;
    let out_arr_d = &*out_arr;
    while lo < hi {
        let e = *region_array::read(out_arr_d, out_lo);
        region_array::replace(arr_d, lo, e);
        out_lo += 1;
        lo += 1;
    }
    Ok(())
...
mod permissions_array {
    ...
    use crate::sorts::verus_no_g_less_arcs::PCell;
