mod pcell {
    ...
    impl<V> PCell<V> {
        ...
        pub fn replace(&self, in_v: V) -> V {
            unsafe { std::mem::replace(&mut *(&self.ucell as *const V as *mut V), in_v) }
        }
...
pub mod exec_pcell {
    use crate::merge_sorts::verus_changed3::pcell::PCell;
    pub struct Array<T> {
        ptrs: Vec<PCell<T>>,
    }
    impl<T> Array<T> {
...
use crate::merge_sorts::verus_changed3::exec_pcell::Array;
