...
pub mod exec_pcell {
    use crate::merge_sorts::verus_without_ghost_profiled::pcell::PCell;
    pub struct Array<T> {
        ptrs: Vec<PCell<T>>,
    }
    impl<T> Array<T> {
...
use crate::merge_sorts::verus_without_ghost_profiled::exec_pcell::Array;
...
pub mod mergesort {
    use crate::merge_sorts::parallel_profiled::{update_stats, Stats};
    ...
    fn merge_sort_parallel(
        ...
        std::thread::scope(|scope| {
            let left_perms = scope.spawn(move || -> Result<(), ()> {
                let t = merge_sort_parallel(arr_r1, lo, mid, out_arr_r1, out_lo, threshold, stats);
                if t.is_err() {
                    return Err(());
                } else {
                    Ok(())
            ...
            match merge_sort_parallel(arr_r2, mid, hi, out_arr_r2, out_mid, threshold, stats) {
                Ok(()) => {}
                Err(_) => {
